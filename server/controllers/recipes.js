...
const getRecipes=(req,res)=>{
    db.query(`SELECT r.recipes_id, r.title, r.image_url, r.body, r.image_id, u.user_name, co.occasion, ct.type, cs.diet,
...
const getRecipesFiltered=(req,res)=>{
    ...
    db.query(`SELECT r.recipes_id, r.title, r.image_url, r.image_id, r.body, u.user_name, co.occasion, ct.type, cs.diet, r.categ_occ_id, r.categ_types_id, r.categ_spec_id 
                FROM recipes r
                left join categ_occ co ON r.categ_occ_id = co.categ_occ_id
                left join categ_types ct ON ct.categ_types_id = r.categ_types_id
                left join categ_special cs ON cs.categ_special_id = r.categ_spec_id
...
const createRecipe=async (req,res)=>{
    ...
    const fileSize=1024
    if(!fileTypes.includes(image_url.mimetype)) return res.send('Támogatott képformátumok: jpg, png, jpeg');
    if(image_url.size/1024>fileSize) return res.send(`A kép mérete nem lehet nagyobb, mint ${fileSize}kb`)
...
const deleteRecipe=(req,res) => {
    ...
    db.query('DELETE from recipes_ingr where recipes_id=?',[id],(err,result)=>{
        if(err){
            console.log('Hiba a törlésben:',err)
            res.send({message:`Hiba a törlés során:${err}`})
        }
        if(result)
            db.query('DELETE from recipes where recipes_id=?',[id],
                (err,result)=>{
                    if(err){
                        res.send({message:`A törlés sajnos nem sikerült! - ${err}`})
                    }
                    if(result){
